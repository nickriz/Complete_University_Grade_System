...
<body>
  ...
  <form action="diaxeirisi_mathimaton_admin.php?task=edit&course_id=<?php echo $course_id;?>" method="post">
        <?php

$link= mysqli_connect("localhost","root","","pegasus");

$sql = "SELECT c.course_id, c.title, c.semester, c.theory_grade_percentage, c.labratory_grade_percentage, c.description, p.professor_id  FROM courses c, professors p WHERE course_id= '$course_id' AND c.professor_id = p.professor_id";
...
while($row = $result->fetch_assoc()){
	?>
        ...
        <div class="form-group row">
          ...
          <input type="text" class="col-4 form-control" id="InputID" name="InputID" value ="<?php echo $row['course_id'];?>">
        </div>
        <div class="form-group row">
          <label class="col-2 col-form-label" for="InputTitle">Τίτλος: </label>
          <input type="text" class="col-4 form-control" id="InputTitle" name="InputTitle" value ="<?php echo $row['title'];?>">
        </div>
        <div class="form-group row">
          <label class="col-2 col-form-label" for="InputSemester">Εξάμηνο: </label>
          <input type="text" class="col-4 form-control" id="InputSemester" name="InputSemester" value =" <?php echo $row['semester'];?>">
        </div>
        <div class="form-group row">
          <label class="col-2 col-form-label" for="InputThGrade">Ανατεθειμένο σε: </label>
          <input type="text" class="col-4 form-control" id="InputThGrade" name="ProfessorID" value = " <?php echo $row['professor_id'];?>"	>
        </div>
		 <div class="form-group row">
          <label class="col-2 col-form-label" for="InputThGrade">Βαθμός Θεωρίας: </label>
          <input type="text" class="col-4 form-control" id="InputThGrade" name="InputThGrade" value = " <?php echo $row['theory_grade_percentage'];?>"	>
        </div>
        <div class="form-group row">
          <label class="col-2 col-form-label" for="InputLabGrade">Βαθμός Εργαστηρίου: </label>
          <input type="text" class="col-4 form-control" id="InputLabGrade" name="InputLabGrade" value = " <?php echo $row['labratory_grade_percentage'];?>">
        </div>
        <div class="form-group row">
          <label class="col-2 col-form-label" for="InputDescription">Περιγραφή Μαθήματος: </label>
          <textarea type="text" class="col-4 form-control" id="InputDescription" name="InputDescription" value = " <?php echo $row['description'];?>" rows="3">
          </textarea>
        </div>
