...
<body>
  ...
  <div class="page-content content-photo p-4" id="content">
    ...
    <div class="col-lg-12">
      <h3 class="mathimata">Νέα Δήλωση Μαθημάτων</h3><br>
      <form class="" action="oristikopoihsh.php" method="post">
        ...
        <tbody>
          ...
          $student_id = $_SESSION['student_id'];
			        ...
			        $sql = "SELECT c.course_id, c.title, p.first_name, p.last_name , c.semester
                    FROM courses c, course_requirements cr, passed_courses pc , professors p
			              WHERE (pc.student_id = '$student_id')
			              AND ( cr.course_id_chain = pc.course_id AND cr.course_id_main = c.course_id
                    AND c.professor_id = p.professor_id AND cr.course_id_main != pc.course_id)
			              OR
			              (cr.course_id_chain IS NULL AND cr.course_id_main = c.course_id
                    AND c.professor_id = p.professor_id AND cr.course_id_main != pc.course_id)";
                    ...
                    if ($result->num_rows > 0) {
                      ...
                      while($row = $result->fetch_assoc()){
                        ...
                        <th> <input id="cb" type="checkbox" name = "courses[]" value="<?php echo $row['course_id']; ?>">  </th>
                      </tr>
      ...
      <div class="row">
        <div class="col-12" style="padding-left :2px;">
          <button class="btn text-dark float-center bg-gray" type="submit"
           name="prosorini_upovoli">Προσωρινή υποβολή δήλωσης</button>
           <button class="btn text-dark float-right bg-gray"  type="submit"
            name="oristiki_upovoli">Οριστική υποβολή δήλωσης</button>
      </div>
      </form>
    </div>
    <br />
